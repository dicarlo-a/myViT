...
\begin{enumerate}[label=(\alph*)]
    ...
    & Count \\
    \midrule
    Total parameters         & 10,755,117 \\
    Trainable parameters     &    275,373 \\
    \quad of which LoRA A/B  &    258,048 \\
    \quad of which head      &     17,325 \\
    Trainable / Total        &      2.56\% \\
    \bottomrule
    \end{tabular}
    \end{center}

    The LoRA count follows from 36 adapted heads (6 heads $\times$ 6 blocks), each contributing $2 \times (r \cdot d_\text{model} + d_\text{head} \cdot r) = 2 \times (8 \cdot 384 + 64 \cdot 8) = 7{,}168$ parameters, giving $36 \times 7{,}168 = 258{,}048$.
    ...
    \midrule
    Linear probe & \textit{TBD} & 17,325 & \textit{TBD} & \textit{TBD} \\
    LoRA ($r=8$, $\alpha=16$) & \textit{TBD} & 275,373 & \textit{TBD} & \textit{TBD} \\
    Full fine-tuning & \textit{TBD} & 10,755,117 & \textit{TBD} & \textit{TBD} \\
    \bottomrule
    \end{tabular}
    \end{center}

    \textit{Discussion TBD after Colab training run.}
    ...
    \textit{Discussion TBD after Colab rank-sweep run.}
